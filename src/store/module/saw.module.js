...
const saw = {
  ...
  // },
  mutations: {
    setSAWResult: (state, payload) => {
      state.sawResults = payload;
    },
  ...
  actions: {
    async fetchSAWResult(context) {
      try {
        const { data } = await axios.post(
          `${process.env.VUE_APP_API_URL}/user-question-answer/calculate-saw`
        );
        context.commit("setSAWResult", data);
      } catch (error) {
